...
public class RollupHandlersIntegrationTest extends IntegrationTestBase {
    ...
    private void testGetRollupByPoints() throws Exception {
        ...
        testThriftRollupHandlerGetByPoints(answers, points, baseMillis, baseMillis + 86400000);
    ...
    private void testGetRollupByResolution() throws Exception {
        for (Locator locator : locators) {
            testThriftAndHTTPHandlersGetByResolution(locator, Resolution.FULL, baseMillis, baseMillis + 86400000, 1440);
            testThriftAndHTTPHandlersGetByResolution(locator, Resolution.MIN5, baseMillis, baseMillis + 86400000, 289);
            testThriftAndHTTPHandlersGetByResolution(locator, Resolution.MIN20, baseMillis, baseMillis + 86400000, 73);
            testThriftAndHTTPHandlersGetByResolution(locator, Resolution.MIN60, baseMillis, baseMillis + 86400000, 25);
            testThriftAndHTTPHandlersGetByResolution(locator, Resolution.MIN240, baseMillis, baseMillis + 86400000, 7);
            testThriftAndHTTPHandlersGetByResolution(locator, Resolution.MIN1440, baseMillis, baseMillis + 86400000, 2);
        }
    }

    public void testThriftRollupHandlerGetByPoints(Map<String, Integer> answers, Map<String, Integer> points,
                                                   long from, long to) throws Exception {
        for (Locator locator : locators) {
            for (Granularity g2 : Granularity.granularities()) {
                List<RollupMetric> rms = thriftRollupHandler.GetDataByPoints(
    ...
    private void testThriftAndHTTPHandlersGetByResolution(Locator locator, Resolution resolution, long from, long to,
                                                         int expectedPoints) throws Exception {
        Assert.assertEquals(expectedPoints, getNumberOfPointsViaThriftHandler(thriftRollupHandler, locator,
                from, to, resolution));
        Assert.assertEquals(expectedPoints, getNumberOfPointsViaHTTPHandler(httpHandler, locator,
                from, to, resolution));
    }

    private int getNumberOfPointsViaThriftHandler(ThriftRollupHandler handler, Locator locator,
                                                 long from, long to, Resolution resolution) throws Exception {
        return handler.GetDataByResolution(locator.getAccountId(), locator.getMetricName(),
