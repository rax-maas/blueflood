...
class ZKShardLockManager implements ConnectionStateListener, ShardLockManager, ZKShardLockManagerMBean {
    ...
    /** Number of outstanding acquire & release operations. */
    private final AtomicInteger lockTaskCount = new AtomicInteger(0);
    ...
    private final Meter lockAcquisitionFailure = Metrics.meter(ZKShardLockManager.class, "Lock acquisition failures");
    private final com.codahale.metrics.Timer lockAcquisitionTimer = Metrics.timer(ZKShardLockManager.class, "Lock acquisition timer");
    private final Meter lockErrors = Metrics.meter(ZKShardLockManager.class, "Lock errors");

    ZKShardLockManager(String zookeeperCluster, Set<Integer> managedShards) {
        try {
    ...
    /**
     * Initialize the ZKShardLockManager.
    ...
    boolean holdsLockUnsafe(int shard) {
        return locks.containsKey(shard) && locks.get(shard).isHeld();
    }

    @VisibleForTesting
    boolean releaseLockUnsafe(int shard) throws Exception {
        return this.release(shard);
    }

    @VisibleForTesting
    void shutdownUnsafe() throws Exception {
        for (Lock lock : locks.values()) {
            this.release(lock.getShard());
        }
        client.close();
    }
    ...
    public synchronized boolean release(int shard) {
        if (locks.containsKey(shard)) {
            try {
                return locks.get(shard).release(false).get();
            } catch (InterruptedException ex) {
    ...
    public synchronized boolean acquire(int shard) {
        if (locks.containsKey(shard)) {
            try {
                return locks.get(shard).acquire().get();
            } catch (InterruptedException ex) {
                log.error("Thread is interrupted: "+ ex.getMessage(), ex);
                return false;
            } catch (ExecutionException ex) {
    ...
    enum LockState {
        /** Locks in UNKNOWN state will be attempt to be acquired. */
        UNKNOWN,
        ...
        ACQUIRE_FAILED,
        /**
         * Error state. normally means that the connection is lost. {@link Lock#mutex} is set to null.
         */
        ERROR,
    ...
    class Lock {
        ...
        // Timestamp of when the mutex has switched.
        ...
        @Override public String toString() {
            return String.format("shard=%d state=%s", shard, state);
        }
        ...
        boolean isHeld() { return connected && mutex != null && state == LockState.ACQUIRED; }
        boolean isUnheld() { return connected && mutex != null && state == LockState.ACQUIRE_FAILED; }
        ...
        boolean performMaintenance(boolean shouldAttempt) {
            ...
            if (state == LockState.UNKNOWN && shouldAttempt) {
                ...
                if (now == stateChanged) {
                    ...
                    chance = (float)(now - stateChanged - minLockHoldTime.toMillis()) / (float)(Math.max(1, minLockHoldTime.toMillis()));
                }
        ...
        /**
         * Only work on the acquired shards. Technically, working on the {@link LockState#ERROR} shard is ok,
         * but it is not recommended as it's going to cause a load problem.
         */
        boolean canWork() {
        ...
        synchronized void setState(LockState newState) {
            state = newState;
            if (newState == LockState.ERROR) {
                if (mutex.isAcquiredInThisProcess()) {
                    log.error("Assertion failure. You're still holding the lock in an error state.");
                }
                // clear the mutex if we reach the error state.
                mutex = null;
            }
        ...
        Future<Boolean> release(boolean disconnected) {
            try {
                log.debug("Releasing lock for shard={}.", shard);
                lockTaskCount.incrementAndGet();
                return lockWorker.submit(releaser(disconnected));
            } catch (RejectedExecutionException ex) {
                log.warn(String.format("Rejected lock execution: active:%d queue:%d shard:%d", lockWorker.getActiveCount(), lockWorker.getQueue().size(), shard));
                return Futures.immediateFuture(false);
            }
                    ...
                    synchronized (Lock.this) {
                        try {
                            ...
                            if (state == LockState.ACQUIRED) {
                                ...
                                // Prior acquisition have failed. Do not attempt.
                                return false;
                            } else {
                                log.debug("Trying ZK lock for shard={}", shard);
                                if (mutex.isAcquiredInThisProcess()) {
                                    // this should not occur but is here as a safeguard.
                                    log.error("Lock already acquired for shard={}", shard);
                                    ...
                                    try {
                                        ...
                                        log.error("Exception on ZK acquire for shard={}", shard);
                                        lockErrors.mark();
         ...
         * @param disconnected If we're releasing the lock due to a ZooKeeper connection loss.
         */
        Callable<Boolean> releaser(final boolean disconnected) {
            return new Callable<Boolean>() {
                public Boolean call() throws Exception {
                    synchronized (Lock.this) {
                        try {
